...
Usuario de movilidad sostenible
...
```env
# MongoDB
MONGODB_URI=mongodb://localhost:27017/mobilitygreen_db
...
```env
MONGODB_URI=mongodb+srv://usuario:password@cluster.mongodb.net/mobilitygreen_db
...
```json
{
  ...
  "licenciaConducir": "Valor de ejemplo"
,
  "createdAt": "2024-01-01T00:00:00.000Z",
...
## 📸 Crear Vehiculos con Imágenes

### ⚠️ IMPORTANTE: La imagen es OPCIONAL

La imagen **NO es requerida** al crear un vehiculo. Puedes:
- ✅ Crear el vehiculo SIN imagen
- ✅ Agregar la imagen DESPUÉS usando el endpoint de upload

### 🎯 Flujo Recomendado (Paso a Paso)

#### Paso 1: Crear el Vehiculo sin imagen
...
{
  "modelo": "Bicicleta Eléctrica Urban",
  "marca": "EcoRide",
  "tipo": "Bicicleta",
  "autonomia": 50,
  "disponible": true
...
**💡 Nota:** Guarda el `_id` del vehiculo creado, lo necesitarás para subir la imagen.

---

#### Paso 2: Subir imagen al Vehiculo

```bash
POST /api/vehiculo/507f1f77bcf86cd799439011/upload-image
Authorization: Bearer {{token}}
...
8. **Copiar** el `_id` del vehiculo creado

---

#### Paso 2: Subir Imagen

1. **Ir a:** `Upload por Entidad` → `Upload Vehiculo Image`
2. **Reemplazar** `{{id}}` en la URL con el ID copiado:
   ```
   http://localhost:3020/api/vehiculo/507f1f77bcf86cd799439011/upload-image
   ```
...
**✅ ¡Listo!** Tu vehiculo ahora tiene imagen.
...
**Imagen original:**
```
http://localhost:3020/uploads/1700000000000-imagen.jpg
```

**Thumbnail (miniatura):**
```
http://localhost:3020/uploads/thumbnails/thumb-1700000000000-imagen.jpg
```
...
# 2. Crear Vehiculo
PRODUCTO_ID=$(curl -X POST http://localhost:3020/api/vehiculo \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{   "modelo": "Bicicleta Eléctrica Urban",   "marca": "EcoRide",   "tipo": "Bicicleta",   "autonomia": 50,   "disponible": true }' \
  | jq -r '.data._id')

echo "Vehiculo creado con ID: $PRODUCTO_ID"

# 3. Subir imagen
curl -X POST http://localhost:3020/api/vehiculo/$PRODUCTO_ID/upload-image \
  -H "Authorization: Bearer $TOKEN" \
...
- La URL debe empezar con `http://localhost:3020/uploads/`
...
```bash
curl -X GET http://localhost:3020/api/usuario-profile/me \
...
```bash
curl -X PUT http://localhost:3020/api/usuario-profile/me \
  -H "Authorization: Bearer {access_token}" \
  -H "Content-Type: application/json" \
  -d '{
...
**Archivo:** `mobilitygreen-api.postman_collection.json`
...
├── mobilitygreen-api.postman_collection.json
└── package.json
```
